...
public class Reporter {
    ...
    public int reportMovingObject(int time, long id, int repNum, int objClass, double x, double y, double speed, double doneDist, int nextNodeX, int nextNodeY, int reportProbability) {
        ...
        repNum++;
        if (visualize && (objects != null))
            visualizeMovingObject(id,(int) x, (int) y, objClass, time);
        return repNum;
    }
    ...
    public int reportNewMovingObject(int time, long id, int objClass, int x, int y, double speed, int nextNodeX, int nextNodeY, int reportProbability) {
        ...
        if (reportProbability > 0) {
            ...
            repNum++;
            if (visualize && (objects != null))
                visualizeMovingObject(id,x, y, objClass, time);
        }
        return repNum;
    }
     ...
     * @param time     time stamp
     */
    protected void visualizeMovingObject(long uid, int x, int y, int objClass, int time) {
        DrawableSymbol symbol = new DrawableSymbol(uid, x, y, "Point" + objClass + "-" + time);
        symbol.setLayer(SYMBOLLAYER);
        objects.addDrawable(symbol);
//        symbol = new DrawableSymbol(281, 3935, "Point" + objClass + "-" + time);
//        objects.addDrawable(symbol);
//        symbol = new DrawableSymbol(23853, 30850, "Point" + objClass + "-" + time);
//        objects.addDrawable(symbol);
//        symbol = new DrawableSymbol(26915, 23572, "Point" + objClass + "-" + time);
//        objects.addDrawable(symbol);
//        symbol = new DrawableSymbol(23000, 26500, "Point" + objClass + "-" + time);
//        objects.addDrawable(symbol);
    }

    public DrawableObjects getDrawableObjects() {return objects;}
}