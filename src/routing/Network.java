...
public class Network {
	...
	/**
	 * 	Container f�r Knoten
	 */
	protected Nodes nodes = null;
	/**
	 * 	Container f�r Kanten
	 */
	...
	/**
	 * 	Knoten-Weg-Index f�r Weg1
	 */
	private static final int NWAY1 = 1;
	/**
	 * 	Knoten-Weg-Index f�r Weg2
	 */
	private static final int NWAY2 = 2;
	/**
	 * 	allgemeine Knoten-Markierung f�r Wege
	 */
	private static final int WAY = 1;
	/**
	 * 	Knoten-Markierung f�r Weg1
	 */
	private int WAY1 = NWAY1;
	/**
	 * 	Knoten-Markierung f�r Weg2
	 */
 ...
 * Die Berechnung wird nur vom Startknoten aus durchgef�hrt.
 * @return berechneter Weg
 * @param start Startknoten
 * @param stop Stopknoten
 */
...
public PathEdge computeFastWay2 (Node start, Node stop) {
	...
	while (elementFound) {
		...
		// Fall 1: Knoten geh�rt zu WAY1
		if (actNode.isMarked(WAY1)) {
			actNode.mark(FINAL);
			// falls er auch zu WAY2 geh�rt, sind beide Wege nun aufeinander getroffen => fertig
			if (actNode.isMarked(WAY2))
				break;
			enlargeBorder (border, actNode,stop, WAY1, true);
		}	
		// Fall 2: Knoten geh�rt zu WAY2
 ...
 * schlechter sind als der k�rzeste Weg. <BR>
 * Einschr�nkungen: <BR>
 * - keine Zyklen <BR>
 * - keine R�ckwege (kann evtl. auch ung�nstige Querverbindungen treffen <BR>
 * @return berechneter Weg
 * @param start Startknoten
 * @param stop Stopknoten
...
public PathEdge computeFastWays (Node start, Node stop, int threshold) {
	// Pr�fen, ob Weg berechenbar
	if ((start == null) || (stop == null) || (start.equals(stop)))
		return null;
	// Initialisierung
	border.reset();					// Rand
	PathEdge resPath = null;					// Ergebnis
	double shortestDistance = maxDistance;		// Distanz des k�rzesten Wegs
	...
	for (int numOfLoops=1; true; numOfLoops++) {
		...
		// Abarbeiten und Erg�nzen des Randes
		boolean elementFound = border.fetchFirst();
		while (elementFound) {
			// Pr�fen, ob Weg zu lang => ggf. Durchlauf abbrechen
			if (border.returnDistance()*100/(100+threshold) > shortestDistance)
				break;
			Node actNode = border.returnNode();
			actNode.mark(FINAL);
			// Fall 1: Knoten geh�rt zu NWAY1
			if (actNode.isMarked(WAY1)) {
				...
				// Fall 1b: Knoten geh�rt auch zu NWAY2
				else if (actNode.isMarked(WAY2)) {
...
/**
 * Berechnung der Verbindung zwischen zwei BEST-Knoten, die �bergebenene Kante umfa�t.
 * Die Entfernung zum Zielknoten wird in den Rand-Elementen abgelegt.
 * @param border Rand gem�� Dijkstra als Heap
 ...
 * @param threshold Prozentsatz, um den Wege l�nger sein d�rfen als der k�rzester Weg
 * @param shortestDistance L�nge des k�rzesten Weges
...
protected void computeNewConnection (BorderHeap border, Node start, Edge actEdge, Node stop, int wayMark, int threshold, double shortestDistance, PathEdge resPath) {
	...
	// Endpunkt der Verbindung feststellen und die Wegl�nge dahin berechnen
	long addDistance = 0;
	...
	// Falls beide zuvor BEST-Knoten, pr�fen ob Verbindung in Resultat vorliegt
	if (bothBest) {
		bothBest = (resPath.findEdge (actEdge) != null);
		if (!bothBest) {
			System.out.print("bothBest: "); searchNode.debugPrint(way); System.out.print(" -> "); oppositeNode.debugPrint(way); System.out.print(" bislang nicht in L�sung."); System.out.println();
		}
	}	

	// Falls man �ber die Verbindung vorangekommen ist und den Schwellenwert nicht �berschreitet: Pfad als Ergebnis aufnehmen
 ...
 * @param computeNWAY2 Soll auch der mit NWAY2-Weg berechnet werden?
 */
protected PathEdge computeResultPath (Node firstNode, boolean computeNWAY2) {
	...
	// Ggf. Ergebnis zur�ckgeben	
	if (!computeNWAY2) {
		return path;
	}
		
	// Berechnung NWAY2
	// Fall ber�cksichtigen, da� es keine NWAY2-Kante gibt, 
	if (firstNode.getWayEdge(NWAY2) == null) {
		return path;
	}
	// In Abh�ngigkeit vom NWAY1-Weg Resultat verketten
...
System.err.println("Ich glaube das nachfolgende ist falsch, da sich path nicht �ndert");
 ...
 * @param edgeIn DataInputStream f�r Kanten
 * @param objects Container f�r drawable objects
 */
public void createByNetworkFiles (DataInputStream nodeIn, DataInputStream edgeIn, DrawableObjects objects) {
	...
	int line = 1;
	boolean eof = false;
	HashSet<Node> idSet = new HashSet<>();
	while (!eof) {
		try {
			...
			if (actEdge != null) {
				if (objects != null) {
					...
					Node node1 = actEdge.getNode1();
					Node node2 = actEdge.getNode2();
					if (!idSet.contains(node1)) {
						idSet.add(node1);
						node1.x = (node1.x + node2.x)>>1;
						node1.y = (node1.y + node2.y)>>1;
					}
 ...
 * Diese Version der Methode unterst�tzt die Berechnung von Verbindungen,
 * f�r den Fall, da� die Erweiterung auf einen FINAL- oder BEST-Knoten trifft.
 * Die Entfernung zum Zielknoten wird grunds�tzlich in den Rand-Elementen abgelegt.
 * @param border Rand gem�� Dijkstra als Heap
...
protected void enlargeBorder (BorderHeap border, Node start, Node stop, int wayMark, int threshold, double shortestDistance, PathEdge resPath) {
	// Vorbereitungen
	int way = wayMark / WAY1;
	// �ber alle Kanten des aktuellen Knotens iterieren
	Edge actEdge = start.getFirstEdge();
	while (actEdge != null) {
		// Ausschlie�en, da� Zur�ckgelaufen wird
		if (actEdge != start.getWayEdge(way)) {
			...
			if ((!oppositeNode.isMarked(FINAL)) &&
				...
				// oder Gewicht anpassen (bei akt. Heap-Implementierung ist dazu nichts n�tig)
				else {
					//System.out.print("ge�ndert im Heap: "); oppositeNode.debugPrint(way); System.out.println();
					// Pr�fen, ob �ber alten Weg neue Verbindung anlegbar ist
			...
			// ansonsten, Pr�fen, ob eine neue Verbindung anlegbar ist
 ...
 * @param considerDistToDest Soll die Entfernung zum Zielknoten ber�cksichtigt werden?
 */
