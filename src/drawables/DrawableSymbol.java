...
public class DrawableSymbol extends FilledDrawable {
    ...
    public DrawableSymbol(long uid,int px, int py, String presName) {
        this();
        this.uid = uid;
        x = px;
        y = py;
     ...
     * @param presName  Name des Darstellungsobjekts
     * @param pMinScale Mindest-Ma�stab zum Zeichnen
     * @param pMaxScale Maximal-Ma�stab zum Zeichnen
     */
    ...
    protected void drawProtected(Graphics g, int scale, int mode, int pvalue) {
        ...
        int off = 0;            // Offset f�r Symbol mit eingelagerten Grafik-Primitiven
        if (size % 2 == 1)
            off = 1;
        Polygon pol = null;
        switch (ap.type) {
            case SQUAREDOT:
            case SQUARE:
                g.fillRect(x / scale - size / 2, y / scale - size / 2, size, size);
                break;
            case CIRCLEDOT:
            case CIRCLE:
            ...
            case RING:
                ...
                break;
            case TRIANGLE:
                int pgxT[] = {x / scale - size / 2, x / scale + size / 2, x / scale};
        ...
        switch (ap.type) {
            case SQUAREDOT:
                g.fillRect(x / scale - size / 6, y / scale - size / 6, (size + off) / 3, (size + off) / 3);
            case SQUARE:
                g.drawRect(x / scale - size / 2, y / scale - size / 2, size, size);
                break;
            case RING:
                g.drawOval(x / scale - size * 3 / 10, y / scale - size * 3 / 10, (size + off) * 3 / 5, (size + off) * 3 / 5);
                g.drawOval(x / scale - size / 2, y / scale - size / 2, size, size);
                break;
            case CIRCLEDOT:
                g.drawOval(x / scale - size / 6, y / scale - size / 6, (size + off) / 3, (size + off) / 3);
            case CIRCLE:
                g.drawOval(x / scale - size / 2, y / scale - size / 2, size, size);
                break;
            case TRIANGLE:
    ...
    /**
     * Gibt die x-Koordinate des Symbols zur�ck.
     *
     * @return x-Koordinate
     */
    public int getX() {
    ...
    /**
     * Gibt die y-Koordinate des Symbols zur�ck.
     *
     * @return y-Koordinate
     */
    public int getY() {
