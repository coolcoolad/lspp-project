...
public class DrawableObjects {
    ...
    /**
     * Zeichnet alle Drawable-Objekte im angegebenen Graphic Context, vorausgesetzt der aktuelle Ma�stab
     * wird vom Drawable-Objekt �berschritten und es liegt im �bergebenen Rechteck.
     * Die Zeichenreihenfolge richtet sich nach den Layern.
     *
     * @param g     aktueller Graphic Context
     * @param r     Clipping-Rechteck
     * @param scale aktueller Ma�stab
     */
    ...
    public Drawable getNextDrawable(Drawable prevPrim) {
        int l = 0;
        Drawable next = null;
        // ggf. n�chstes im akt. Layer bestimmen
        if (prevPrim != null) {
        ...
        // ggf. n�chstes in den folgenden Layern bestimmen
        while (l < numOfLayers) {
    ...
    /**
     * Gibt das n�chste benannte, sichtbare und ggf. selektierbare Drawable-Objekt zur�ck,
     * welches sich an der angegebenen Position befindet und den Ma�stab einh�lt.
     * Falls es kein solches Objekt gibt, wird null zur�ckgegeben.
     *
     ...
     * @param scale          Mindest-Ma�stab
     * @param selectable mu� das Objekt selektierbar sein?
    ...
    /**
     * Gibt das Drawable-Objekt zur�ck, welches die �bergebende ID besitzt.
     * Falls es kein solches Objekt gibt, wird null zur�ckgegeben.
     *
    ...
    /**
     * Gibt das Drawable-Objekt zur�ck, welches die �bergebende ID besitzt und sichtbar ist.
     * Falls es kein solches Objekt gibt, wird null zur�ckgegeben.
     *
     * @param id    gesuchte ID
     * @param scale Ma�stab
    ...
    public void readDrawableObject(EntryInput r, String objType) {
        ...
        if (r.eol()) {
            ...
            // ggf. zugeh�rigen Text suchen
            // kann entfallen, wenn alle L�nderdateien umgestellt
    ...
    /**
     * L�scht das Drawable-Objekt aus dem Container und l�st die Beziehung
     * zum Grafik-Primitiv auf.
     *
     * @param obj zu l�schendes Drawable-Objekt
     */
    ...
    /**
     * Deselektiert das bislang selektierte Drawable-Objekt und selektiert das �bergebene Objekt.
     *
     * @param obj zu l�schendes Drawable-Objekt
     */
    ...
    public ArrayList<Drawable> getAllDrawbleObjects() {
        ArrayList<Drawable> arr = new ArrayList<>();
        for(int layer_=0; layer_ < numOfLayers; layer_++){
            Drawable cur = layer[layer_];
            while (cur != null){
                arr.add(cur);
                cur = cur.getNextOfLayer();
            }
        }
        return arr;
    }

    public ArrayList<DrawableSymbol> getAllDrawableSymbols() {
        ArrayList<DrawableSymbol> arr = new ArrayList<>();
        Drawable cur = layer[4];
        while (cur != null){
            arr.add((DrawableSymbol) cur);
            cur = cur.getNextOfLayer();
        }
        return arr;
    }
